import { useEffect, useState } from "react";
import BookModel from "../../../models/BookModel";
import { useAuth } from "../../../Auth/AuthProvider";
import api from "../../../Api/apiClient";

export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: () => void }> = (props) => {

    const { isLoggedIn } = useAuth();
    const [quantity, setQuantity] = useState<number>(0);
    const [remaining, setRemaining] = useState<number>(0);

    useEffect(() => {
        const fetchBookInState = () => {
            setQuantity(props.book.copies ?? 0);
            setRemaining(props.book.copiesAvailable ?? 0);
        };
        fetchBookInState();
    }, []);

    async function increaseQuantity() {
        const url = `/api/admin/secure/increase/book/quantity/?bookId=${props.book?.id}`;
        try {
            await api.put(url);
            setQuantity(prev => prev + 1);
            setRemaining(prev => prev + 1);
        } catch (error) {
            console.error('Failed to increase quantity:', error);
        }
    }

    async function decreaseQuantity() {
        const url = `/api/admin/secure/decrease/book/quantity/?bookId=${props.book?.id}`;
        try {
            await api.put(url);
            setQuantity(prev => prev - 1);
            setRemaining(prev => prev - 1);
        } catch (error) {
            console.error('Failed to decrease quantity:', error);
        }
    }

    async function deleteBook() {
        try {
            const url = `/api/admin/secure/delete/book/?bookId=${props.book?.id}`;
            const response = await api.delete(url);

            if (response.status !== 200) {
                throw new Error('Something went wrong!');
            }

            console.log('Book deleted successfully');
            props.deleteBook(); // 🔄 update parent state
        } catch (error) {
            console.error('Error deleting book:', error);
        }
    }

    return (
        <div className='card mt-3 shadow p-3 mb-3 bg-body rounded'>
            <div className='row g-0'>
                <div className='col-md-2'>
                    <div className='d-none d-lg-block'>
                        <img
                            src={props.book.img || require('../../../Images/BookImages/book-luv2code-1000.png')}
                            width='123' height='196' alt='Book'
                        />
                    </div>
                    <div className='d-lg-none d-flex justify-content-center align-items-center'>
                        <img
                            src={props.book.img || require('../../../Images/BookImages/book-luv2code-1000.png')}
                            width='123' height='196' alt='Book'
                        />
                    </div>
                </div>
                <div className='col-md-6'>
                    <div className='card-body'>
                        <h5 className='card-title'>{props.book.author}</h5>
                        <h4>{props.book.title}</h4>
                        <p className='card-text'> {props.book.description} </p>
                    </div>
                </div>
                <div className='mt-3 col-md-4'>
                    <div className='d-flex justify-content-center align-items-center'>
                        <p>Total Quantity: <b>{quantity}</b></p>
                    </div>
                    <div className='d-flex justify-content-center align-items-center'>
                        <p>Books Remaining: <b>{remaining}</b></p>
                    </div>
                </div>
                <div className='mt-3 col-md-1'>
                    <div className='d-flex justify-content-start'>
                        <button className='m-1 btn btn-md btn-danger' onClick={deleteBook}>Delete</button>
                    </div>
                </div>
                <button className='m-1 btn btn-md main-color text-white' onClick={increaseQuantity}>Add Quantity</button>
                <button className='m-1 btn btn-md btn-warning' onClick={decreaseQuantity}>Decrease Quantity</button>
            </div>
        </div>
    );
}
